...
void analyseOutput()
{
	...
	for (i = 1; i <= intermediatesize; i++)
	{
		...
		else if (intermediatecode[i]->opcode == 14)
		{
			...
			int x = intermediatecode[i]->parameters[0];
			int y=0;
			printf("enter inputt as per your program\n");
			scanf("%d", &y);
		int j = 0;
			for ( j= 0; j < registercodesize; j++)
			{
	...
	printf("\nCODE HAS SUCESSFULLY EXECUTED...................PRESS ANY KEY FOR EXIT.....");
	fflush(stdin);
	char exi;
	scanf("%c", &exi);
}
...
void analyseFile()
{
	printf("\n\n\n\nCan You Please Provide Your Intermediate Code File\n\n\n");
	...
	printf("\n\n\n\nCan You Please Provide Your Symbol Table File\n\n\n");
	char *symbolTableFile = (char *)malloc(100 * sizeof(char));
	scanf("%s", symbolTableFile);
	FILE *fp2 = fopen(symbolTableFile, "r");


	while (fgets(c, 1000, fp2) != NULL)
	{
		char *temp = (char *)malloc(100 * sizeof(char));
		strcpy(temp, c);
		char *variable = (char *)malloc(100 * sizeof(char)); 
		variable=seperator(&temp, " ");
		int x = atoi(seperator(&temp, " "));
		int y = atoi(seperator(&temp, " "));

		addToSymbolTable(variable[0], y, 0);
	}
